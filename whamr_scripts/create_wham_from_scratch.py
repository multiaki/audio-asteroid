...
def create_wham(wsj_root, wham_noise_path, output_root):
    ...
    for splt in SPLITS:
        ...
        for i_utt, output_name in enumerate(utt_ids):
            ...
            room.add_audio(quantize(read_scaled_wav(s1_path, 1)), quantize(read_scaled_wav(s2_path, 1)))
            ...
            for sr_i, sr_dir in enumerate(SAMPLE_RATES):
                ...
                for datalen_dir in DATA_LEN:
                    ...
                    s1_anechoic, s2_anechoic = fix_length(anechoic[sr_i][0, ch_ind, :len(s1)].T * s1_spatial_scaling,
                                                          anechoic[sr_i][1, ch_ind, :len(s2)].T * s2_spatial_scaling,
                                                          datalen_dir)
                    s1_reverb, s2_reverb = fix_length(reverberant[sr_i][0, ch_ind, :len(s1)].T * s1_spatial_scaling,
                                                      reverberant[sr_i][1, ch_ind, :len(s2)].T * s2_spatial_scaling,
                                                      datalen_dir)
